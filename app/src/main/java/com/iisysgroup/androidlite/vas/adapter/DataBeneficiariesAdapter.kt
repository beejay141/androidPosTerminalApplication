...
class DataBeneficiariesAdapter(private var items: ArrayList<DataBeneficiariesModel>, private var context: Context, private var listener: DataClickListener) : RecyclerView.Adapter<DataBeneficiariesAdapter.BeneficiaryViewHolder>() {

    interface DataClickListener {
        fun onDataItemClick(data: ArrayList<DataBeneficiariesModel>, position: Int)
    }
    ...
    override fun onBindViewHolder(holder: BeneficiaryViewHolder, position: Int) {
        ...
        holder.dataTitle.text = String.format("%d. %s", position + 1, items[position].phone_number)
