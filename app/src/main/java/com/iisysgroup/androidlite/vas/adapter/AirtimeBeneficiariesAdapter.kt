...
class AirtimeBeneficiariesAdapter(private var items: ArrayList<AirtimeModel>, private var context: Context, private var listener: DataClickListener) : RecyclerView.Adapter<AirtimeBeneficiariesAdapter.BeneficiaryViewHolder>() {

    interface DataClickListener {
        fun onDataItemClick(data: ArrayList<AirtimeModel>, position: Int)
    }
    ...
    override fun onBindViewHolder(holder: BeneficiaryViewHolder, position: Int) {
        ...
        holder.dataTitle.text = String.format("%d. %s - %s", position + 1, items[position].airtime_provider, items[position].phone_number)
